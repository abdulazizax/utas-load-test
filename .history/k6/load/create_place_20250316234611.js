...
export default function () {
    // Define the API endpoint for creating a place (hotel, hostel, etc.)
    const createPlaceUrl = 'https://api.admin.u-code.io/v2/invoke_function/binn-hotel-create-knative?project-id=7e2c2c8c-522f-4a40-ae43-a43b85ccd32e';
    ...
    // Create place data (e.g., hostel, hotel, property)
    const placeData = {
        "data": {
    ...
    // Perform the POST request to create the place (hotel, hostel, etc.)
    const createPlaceResponse = http.post(createPlaceUrl, JSON.stringify(placeData), { headers: headers });

    // Log the response for debugging
    console.log('Response Status:', createPlaceResponse.status);
    console.log('Response Body:', createPlaceResponse.body);

    // Check if the place creation was successful
    if (createPlaceResponse.status === 200) {
        console.log('Place created successfully!');
    } else {
        console.error(`Failed to create place: ${createPlaceResponse.status}`);
    }
